...
print_stackframe(void) {
	...
	//if ebp not 0 and not reach STACKFRAME_DEPTH limit, Continue to print the information of the stack frame and the corresponding function on the stack in a loop
	for (i = 0; ebp != 0 && i < STACKFRAME_DEPTH; i++)
	{
		...
		//eip pointer to the ready to execute instruction, if want to look the current function, need to sub 1
		print_debuginfo(eip-1);
		
		//set ebp, eip as the before ebp, eip
		//watch to set eip first and ebp later, or when ebp is modified, eip can't find the correct ebp
		eip = *((uint32_t *)ebp + 1);
		ebp = *(uint32_t *)ebp;
	}
}
